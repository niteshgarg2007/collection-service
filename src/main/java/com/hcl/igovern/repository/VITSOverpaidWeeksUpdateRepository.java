...
public interface VITSOverpaidWeeksUpdateRepository extends JpaRepository<VITSOverpaidWeeksUpdateEO, Long> {
	...
	@Query(value = "SELECT * FROM V_ITS_OVERPAID_WEEKS_UPDATE WHERE VICTIM_BAD_ACTOR_XREF_ID=?1 AND CLM_ID=?2 AND IS_CANCELLED=?3 AND (OVP_ID=?4 OR OVP_ID IS NULL)", nativeQuery = true)
